# AWS CodePipeline Plugins Documentation

This document provides a comprehensive overview of the AWS CodePipeline plugins within the CloudSploit system.

## Architecture Overview

The CodePipeline plugins are part of the AWS plugin suite for CloudSploit. They are designed to integrate with the core scanning engine (`engine.js`) to assess the configuration and security of AWS CodePipeline pipelines. The architecture follows the standard CloudSploit plugin pattern, where collectors gather data from AWS, and plugins analyze that data to identify potential security risks.

The primary plugins for CodePipeline are:
1.  **`pipelineHasTags.js`**: Checks if CodePipeline pipelines have tags.
...
graph TD
    ...
    B --> C{AWS CodePipeline Data};
    C --> D{CodePipeline Plugins};
    subgraph CodePipeline Plugins
        direction LR
        E[pipelineHasTags.js]
    end
    D --> E;
    E --> F{Results};
    F --> G[Post-processors];
...
### Use Case 1: Ensure Proper Tagging for CodePipeline Pipelines
- **User Interaction:** The user runs a scan on their AWS account.
- **System Process:**
    - The `pipelineHasTags.js` plugin is executed.
    - It checks the tags of each CodePipeline pipeline.
    - If a pipeline is missing tags, it generates a "FAIL" result.
- **Expected Outcome:** The user is alerted to any CodePipeline pipelines that are not properly tagged, which is important for cost allocation, automation, and access control.

## System Diagrams

### Sequence Diagram: CodePipeline Scan

```mermaid
sequenceDiagram
    participant engine as engine.js
    participant collectors as AWS Collectors
    participant plugins as CodePipeline Plugins
    participant output as output.js

    engine->>collectors: Run collectors
    collectors->>plugins: Provide CodePipeline data
...
-   **AWS SDK:** Used by the collectors to interact with the AWS API and retrieve CodePipeline data.
