# AWS Backup Plugins Documentation

This document provides a comprehensive overview of the AWS Backup plugins within the CloudSploit system.

## Architecture Overview

The Backup plugins are part of the AWS plugin suite for CloudSploit. They are designed to integrate with the core scanning engine (`engine.js`) to assess the configuration and security of AWS Backup plans. The architecture follows the standard CloudSploit plugin pattern, where collectors gather data from AWS, and plugins analyze that data to identify potential security risks.

The primary plugins for Backup are:
1.  **`backupPlanHasTags.js`**: Checks if Backup plans have tags.
2.  **`backupVaultHasTags.js`**: Checks if Backup vaults have tags.
...
graph TD
    ...
    B --> C{AWS Backup Data};
    C --> D{Backup Plugins};
    subgraph Backup Plugins
        direction LR
        E[backupPlanHasTags.js]
        F[backupVaultHasTags.js]
    end
    D --> E;
    D --> F;
    E --> G{Results};
...
### Use Case 1: Ensure Proper Tagging for Backup Resources
- **User Interaction:** The user runs a scan on their AWS account.
- **System Process:**
    - The `backupPlanHasTags.js` and `backupVaultHasTags.js` plugins are executed.
    - They check the tags of each Backup plan and vault.
    - If a resource is missing tags, it generates a "FAIL" result.
- **Expected Outcome:** The user is alerted to any Backup resources that are not properly tagged, which is important for cost allocation, automation, and access control.

## System Diagrams

### Sequence Diagram: Backup Scan

```mermaid
sequenceDiagram
    participant engine as engine.js
    participant collectors as AWS Collectors
    participant plugins as Backup Plugins
    participant output as output.js

    engine->>collectors: Run collectors
    collectors->>plugins: Provide Backup data
...
-   **AWS SDK:** Used by the collectors to interact with the AWS API and retrieve Backup data.
