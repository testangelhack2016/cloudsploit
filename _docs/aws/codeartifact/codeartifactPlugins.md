# AWS CodeArtifact Plugins Documentation

This document provides a comprehensive overview of the AWS CodeArtifact plugins within the CloudSploit system.

## Architecture Overview

The CodeArtifact plugins are part of the AWS plugin suite for CloudSploit. They are designed to integrate with the core scanning engine (`engine.js`) to assess the configuration and security of AWS CodeArtifact repositories. The architecture follows the standard CloudSploit plugin pattern, where collectors gather data from AWS, and plugins analyze that data to identify potential security risks.

The primary plugins for CodeArtifact are:
1.  **`domainHasTags.js`**: Checks if CodeArtifact domains have tags.
2.  **`repositoryHasTags.js`**: Checks if CodeArtifact repositories have tags.
...
graph TD
    ...
    B --> C{AWS CodeArtifact Data};
    C --> D{CodeArtifact Plugins};
    subgraph CodeArtifact Plugins
        direction LR
        E[domainHasTags.js]
        F[repositoryHasTags.js]
    end
    D --> E;
    D --> F;
    E --> G{Results};
...
### Use Case 1: Ensure Proper Tagging for CodeArtifact Resources
- **User Interaction:** The user runs a scan on their AWS account.
- **System Process:**
    - The `domainHasTags.js` and `repositoryHasTags.js` plugins are executed.
    - They check the tags of each CodeArtifact domain and repository.
    - If a resource is missing tags, it generates a "FAIL" result.
- **Expected Outcome:** The user is alerted to any CodeArtifact resources that are not properly tagged, which is important for cost allocation, automation, and access control.

## System Diagrams

### Sequence Diagram: CodeArtifact Scan

```mermaid
sequenceDiagram
    participant engine as engine.js
    participant collectors as AWS Collectors
    participant plugins as CodeArtifact Plugins
    participant output as output.js

    engine->>collectors: Run collectors
    collectors->>plugins: Provide CodeArtifact data
...
-   **AWS SDK:** Used by the collectors to interact with the AWS API and retrieve CodeArtifact data.
