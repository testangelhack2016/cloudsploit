# Azure Plugins and Post-Processors Architecture

This document provides a comprehensive overview of the architecture of Azure plugins and post-processors, including their components, interactions, and technologies.

## Architecture Overview

The system's architecture for Azure plugins and post-processors is designed to be modular and extensible. After the Azure collector gathers data from the Azure environment, the `engine.js` passes this data to the Azure plugins for analysis.

Each Azure plugin is a self-contained module that checks for a specific security vulnerability. The plugins are located in the `plugins/azure` directory. After the plugins have been executed, the results are passed to the post-processors.
...
### Use Case: Analyze Azure Data and Generate Report

1.  **System Process:**
    *   `engine.js` passes the collected Azure data to the Azure plugins.
    *   Each Azure plugin analyzes the data for a specific security issue and returns a result (pass, fail, or unknown).
...
2.  **Expected Outcome:** A formatted report is generated and displayed to the user, showing the security posture of their Azure environment.
...
sequenceDiagram
    ...
    engine.js->>Azure Plugins: Analyze data
    Azure Plugins-->>engine.js: Return results
...
-   **Core Modules:**
    ...
    -   `plugins/azure/*`: Individual Azure security check plugins.
